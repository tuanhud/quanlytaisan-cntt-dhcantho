<script src="../js/yui-min_3.5.js"></script> 
<script src="../js/intl-min.js"></script>
<link rel="stylesheet" href="../css/node-menunav.css"/>
<div id="chsv" class="yui3-menu yui3-menu-horizontal yui3-menubuttonnav">
            <div class="yui3-menu-content">
                <ul class="first-of-type">                    
                    <li>
                        <a class="yui3-menu-label" href="#thongke"><em>Văn phòng phẩm</em></a>
                        <div id="thongke" class="yui3-menu">
                            <div class="yui3-menu-content">
                                <ul>
                                    <li class="yui3-menuitem">
                                        <a class="yui3-menuitem-content" href="capnhatphieudutru.php">Cập nhật phiếu dự trù TB</a>  
                                    </li>
                                </ul>
                            </div>
                        </div>
                    </li>
                    
                    
                    <li>
                        <a class="yui3-menu-label" href="#timkiem"><em>Duyệt cấp bộ môn</em></a>
                        <div id="#timkiem" class="yui3-menu">
                            <div class="yui3-menu-content">
                                <ul>
                                    <li class="yui3-menuitem">
										<a class="yui3-menuitem-content" href="capnhatlchsv.php">Duyệt yêu cầu mua sắm</a>
									</li>
                                    <li class="yui3-menuitem">
										<a class="yui3-menuitem-content" href="capnhatlchsv.php">Báo cáo tài sản bị hư hỏng</a>
									</li>         
                                </ul>
                            </div>
                        </div>
                    </li>
                    
                    <li>
                        <a class="yui3-menu-label" href="#report"><em>Report</em></a>
                        <div id="report" class="yui3-menu">
                            <div class="yui3-menu-content">
                                <ul>
                                    <li class="yui3-menuitem"><a class="yui3-menuitem-content" href="#">In danh sách các yêu cầuđã duyệt</a></li>
									<li class="yui3-menuitem"><a class="yui3-menuitem-content" href="#">In danh sách các yêu cầu cần trao đổi thêm</a></li>
                                   
                                </ul>
                            </div>
                        </div>
                    </li>

                </ul>
            </div>
</div>