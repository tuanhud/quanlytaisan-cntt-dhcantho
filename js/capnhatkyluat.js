...
numofFai = 0;		
// JavaScript Document
function themHoivien_Kyluat(filephp,frm, hoivien_id)
{
	var kyluat_id = frm.cbo_tenkyluat.value;
...
//	try{
		...
		chks.each( function(item){
			...
			themHoivien_Kyluat('themhoivien_kyluat.php',document.frm_capnhatkyluat,rec.get('id') );
		    alert(rec.get('id'));
		   	});
...
//	{
    ...
    dt = new Y.DataTable({
        columns: cols,
        data   : data,
        summary: 'Danh sách hội viên bị kỷ luật',
        caption: 'Danh sách hội viên bị kỷ luật',
        render : '#mytable'
    });		
	}		
	flag = 1;
