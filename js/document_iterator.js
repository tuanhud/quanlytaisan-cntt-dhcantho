...
{
  return new SkypeClick2Call.DocumentCursor(rootNode, toStart);
}
 ...
 */
SkypeClick2Call.DocumentCursor = function(rootNode, toStart)
{
/*
...
{
	var newObject = new SkypeClick2Call.DocumentCursor(this.m_treeWalker.root, true);
...
{
	// result
	var result = {resultcode:SkypeClick2Call.FAIL, nodeCompare:0, nodeWithOffsetsCompare:0};
...
{
	var result = {resultCode:SkypeClick2Call.FAIL, node:null, tagWasClosed:false};
	...
	if (this.m_currentNode)
	{
		result.resultCode = SkypeClick2Call.OK;
	...
	while (true)
	{
		...
		if (!parentNode)
		{
			...
			result.resultCode = SkypeClick2Call.EMPTY;
			return result;
		}
 ...
 */
SkypeClick2Call.DocumentIterator = function(rootNode, fromCursor, toCursor, listener)
{
/*
...
*/
	...
	{
		this.m_fromCursor = new SkypeClick2Call.DocumentCursor(rootNode, true);
	}
	if (!this.m_toCursor)
	{
		this.m_toCursor = new SkypeClick2Call.DocumentCursor(rootNode, false);
	}
...
SkypeClick2Call.DocumentIterator.prototype.Fire = function(cursor, node, tagWasClosed)
{
	...
	case node.COMMENT_NODE:
		...
		break;
	case node.TEXT_NODE:
		var offsets = {beginOffset:0, endOffset:SkypeClick2Call.npos};
...
SkypeClick2Call.DocumentIterator.prototype.Iterate = function()
{
	...
	if ((node = this.m_fromCursor.root) && node && node.nodeType == node.TEXT_NODE)
	{
		...
		while (this.m_listener.CanContinue())
		{
			...
			var endOffset = (compareResult.nodeCompare == 0 ? this.m_toCursor.GetBeginOffset() : SkypeClick2Call.npos);
	...
	else
	{
		...
		while (this.m_listener.CanContinue())
		{
			...
			if (moveResult.resultCode != SkypeClick2Call.OK)
			{
		...
		if (moveResult.resultCode == SkypeClick2Call.EMPTY)
		{
			this.m_fromCursor = null;
			this.m_toCursor = null;
			result = SkypeClick2Call.OK;
