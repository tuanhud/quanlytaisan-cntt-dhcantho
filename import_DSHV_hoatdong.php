...
<?php	
	...
	/*if( $_POST['cbo_tenhoatdong']!=-1 
		&& $_POST['txt_diadiem']!=""
		&& $_POST['txta_noidung']!=0
		&& $filename!=NULL){*/	
	...
	for ($i=2;$i<=$rowsnum;$i++) //Duyệt từng hàng, bắt đầu lấy dữ liệu từ hàng 2
	{
		...
		/* Lưu dữ liệu vào DB
		/*---------------------------------------------*/
		$sql = "INSERT INTO `qlhsv1`.`hoatdong_hoivien` (`HOIVIEN_ID`, `CAPHOI_ID`, `HOATDONG_ID`,`HOCKI_ID`, `HOATDONG_NOIDUNG`, `HOATDONG_DIADIEM`, `HOATDONG_TGBATDAU`, `HOATDONG_TGKETTHUC`, `HOATDONG_DONVI`) VALUES('".$data->val($i,3)."',1,'".$_POST['cbo_tenhoatdong']."', '".$_POST['cbo_tenhocki']."', '".$_POST['txta_noidung']."','".$_POST['txt_diadiem']."','".date( 'y-m-d', strtotime($_POST['cbo_thangbd']."/".$_POST['cbo_ngaybd']."/".$_POST['cbo_nambd']))."','".date( 'y-m-d', strtotime($_POST['cbo_thangkt']."/".$_POST['cbo_ngaykt']."/".$_POST['cbo_namkt']))."', 0)";
