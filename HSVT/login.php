<?php
	session_start();
	if(session_is_registered("maquyen") && $_SESSION['maquyen']=="BCHGSVT")
	{
	...
	{   		
		...
		if ($db->numRecord()==1)
		{
		 //Kiem tra xem nguoi dung co quyen Admin hay khong ?    
				$sql="Select * from hoivien 
				where HOIVIEN_ID = '".$_POST['txtDinhDanh']."' 
				and Quyen_ID = 'BCHHSVT'";
				$db->setQuery($sql);
			if($db->numRecord()==1)
			{
			//dua maso vao session
			$mshsvt =$_POST['txtDinhDanh'];
			session_register("mshsvt") ;
			$sql = "Select HOIVIEN_HOTEN from hoivien where HOIVIEN_ID = '".$mshsvt."'";			
			$db->setQuery($sql);
			$result = $db->fetchAll();
			...
			{
				echo "<script language=javascript> alert('Bạn không có quyền của Ban chấp hành Hội sinh viên trường.');window.location = 'loginUI.php';</script>";
				}
		}
		else
		{
