...
function taobangkiemke(frm,donvi,loaikiemke,phieumau)
{
	 		var data = {};
			var data3 = {};
			var theme = '';
...
{	
			...
			$('#donvi').bind('select', function (event) 
			{
				if (event.args) 
				{
					...
					if (item) 
					{
						...
						taobangkiemke(document.frm_themphieukiemke,donvi,loaikiemke,phieumau);
			...
			$("#btn_themphieukiemke").bind('click', function () 
			{
				if((loaikiemke!=-1)&&(phieumau!=-1)&&(donvi!=-1))
				{
					...
					http=GetXmlHttpObject();
					var params = "Nam=" +nam+"&MaLoaiKK=" +loaikiemke+"&DienGiaiKiemKe=" +diengiai+"&NgayKiemKe=" +ngaybd+"&NgayKetThucKiemKe=" +ngaykt+"&MaPhieu=" +phieumau;
						...
						http.onreadystatechange = function()
						{
							if(http.readyState == 4 && http.status == 200) 
							{
								...
								{	
									...
									http=GetXmlHttpObject();
									var params = "mataisan=" +mats+"&maphieukiemke=" +maphieukiemke+"&manoidung=" +manoidung+"&chitietnoidung=" +chitietnoidung;
									alert(params);
