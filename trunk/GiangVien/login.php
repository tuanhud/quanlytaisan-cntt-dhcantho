	...
	{   		
		...
		if ($db->numRecord()==1)
		{
		 //Kiem tra xem nguoi dung co quyen Admin hay khong ?    
			...
			$sql = "Select HOIVIEN_HOTEN from hoivien where HOIVIEN_ID = '".$msclb."'";			
			$db->setQuery($sql);
			$result = $db->fetchAll();
			...
			{
				echo "<script language=javascript> alert('Bạn không có quyền của Ban chủ nhiệm Câu lạc bộ.');window.location = 'loginUI.php';</script>";
				}
		}
		else
		{
