		...
		$("#save").bind('click', function () 
 		...
 		$("#create").bind('click', function () 
		 {
					...
					//them dl vao bang kehoachmuasam
					{
								http=GetXmlHttpObject();
								var params ="";				
								http.open("POST", 'data_taisanthuocKHMS4.php', false);
								http.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
								http.onreadystatechange = function()
								{
									if(http.readyState == 4 && http.status == 200)
									{
										id = http.responseXML.getElementsByTagName('RESULT')[0].firstChild.nodeValue;	
									} 
								}
								http.send(params);						
					}
					//them du lieu vao bang cokhms
					for(var i=0;i < rowscount;i++) 
					{
						   	var mats = $('#jqthem').jqxGrid('getcellvalue', i, "MaTaiSan");
						   	var sl = $('#jqthem').jqxGrid('getcellvalue', i, "SoLuong");
							var tm = $('#jqthem').jqxGrid('getcellvalue', i, "ThuyetMinh");
							var dg = $('#jqthem').jqxGrid('getcellvalue', i, "DonGia");
							{
								http=GetXmlHttpObject();
								var params = "insert=true&MaTaiSan="+mats+"&SoLuong="+sl+"&DonGia="+dg+"&ThuyetMinh="+tm+"&MaKHMS="+id;				
